                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                <td class="p30-15 bbrr" style="padding: 50px 30px; border-radius:0px 0px 26px 26px;" bgcolor="#0e264b">
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                        <tr>
                                            <td class="text-footer1 pb10" style="color:#c1cddc; font-family:'Muli', Arial,sans-serif; font-size:16px; line-height:20px; text-align:center; padding-bottom:10px;">Wg Gen Plus - A Web based configuration generator for WireGuard</td>
                                        </tr>
                                        <tr>
                                            <td class="text-footer2" style="color:#8297b3; font-family:'Muli', Arial,sans-serif; font-size:12px; line-height:26px; text-align:center;"><a href="https://gitea.lgharvey.net/Lindsay/wg-gen-plus" target="_blank" class="link" style="color:#66c7ff; text-decoration:none;"><span class="link" style="color:#66c7ff; text-decoration:none;">More info coming to Github soon</span></a></td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>
...
AllowedIPs = {{ StringsJoin (AppendStrings .Address .LANIPs) ", " }}
{{- end }}
{{ end }}`
)
...
func DumpServerWg(clients []*model.Client, server *model.Server, preUpHook, postUpHook, preDownHook, postDownHook, WgConfigFile string) ([]byte, error) {
	// Create a copy of clients to avoid modifying the original slice
	...
	t, err := template.New("server").Funcs(template.FuncMap{
		...
		"AppendStrings": func(a, b []string) []string {
			return append(a, b...)
		},
	}).Parse(wgTpl)
	if err != nil {
	...
	configDataWg, err := dump(t, struct {
		...
		Clients:      sortedClients, // Use sorted clients
		Server:       server,
		PreUpHook:    preUpHook,
		PostUpHook:   postUpHook,
		PreDownHook:  preDownHook,
		PostDownHook: postDownHook,
	})
	if err != nil {
		return nil, err
	}

	err = util.WriteFile(WgConfigFile, configDataWg)
	if err != nil {
		return nil, err
	}

	// Reload the WireGuard configuration after writing the file
	err = util.ReloadServerConfig()
	if err != nil {
		return nil, err
	}
